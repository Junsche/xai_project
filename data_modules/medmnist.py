...
def get_loaders(cfg):
    ...
    # mean/std with channel-aware broadcasting
    ...
    if aug_key not in REGISTRY:
        raise KeyError(
            f"Unknown augmentation key for MedMNIST: {aug_key}. "
            f"Available: {list(REGISTRY.keys())}"
        )

    # model-aware preprocessing
    model_input_size = int(model_cfg.get("input_size", data_cfg.get("img_size", 28)))
